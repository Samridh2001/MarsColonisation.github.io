async function aStar(state, diagonalMovement = false){
    ...
    while(!openList.isEmpty()){
        ...
        for(let i=0;i<neighbour.length;i++){
            ...
            if((ny === state.end.y)&&(nx === state.end.x)){
                ...
                let hNew = heuristicVal([ny, nx], [state.end.y, state.end.x], diagonalMovement, state.heuristic);
                let fNew = gNew + hNew;
    ...
    if(foundDest){
        var distance = getPath(state, cellDetails);
  ...
  function getPath(state, cellDetails){
    ...
    while(!((cell[3] === row) && (cell[4] === col))){
         ...
         let dist1 = Math.abs(row-prev1)+Math.abs(col-prev2);
