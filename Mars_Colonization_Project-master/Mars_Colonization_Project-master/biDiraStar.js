async function bidiraStar(state, diagonalMovement = false){
    ...
    for(let i=0;i<state.rows;i++){
        for(let j=0;j<state.cols;j++){
            closedList1[i][j] = 0;
            closedList2[i][j] = 0;
            cellDetails1[i][j] = [maxi,maxi,maxi,-1,-1];
            cellDetails2[i][j] = [maxi,maxi,maxi,-1,-1];
        }
    }
    ...
    while((!openList1.isEmpty())&&(!openList2.isEmpty())){
        ...
        for(let i=0;i<neighbour1.length;i++){
            ...
            if((ny1 === state.end.y)&&(nx1 === state.end.x)){
                ...
                let disstp = Math.abs(ny1-y1)+Math.abs(nx1-x1);
                let gNew1 = cellDetails1[y1][x1][1] + Math.sqrt(disstp);
                let hNew1 = heuristicVal([ny1, nx1], [state.end.y, state.end.x], diagonalMovement, state.heuristic);
        ...
        for(let i=0;i<neighbour2.length;i++){
            ...
            if((ny2 === state.start.y)&&(nx2 === state.start.x)){
                ...
                let distp = Math.abs(ny2-y2)+Math.abs(nx2-x2);
                let gNew2 = cellDetails2[y2][x2][1] + Math.sqrt(distp);
                let hNew2 = heuristicVal([ny2, nx2], [state.start.y, state.start.x], diagonalMovement, state.heuristic);
    ...
    if(foundDest){
        let flag = 0;
        var lis = [];
        getPathbdbest(state, cellDetails1,intersect,lis,flag);
        flag = 1;
        getPathbdbest(state,cellDetails2,intersect,lis,flag);
  ...
  function getPathbdbest(state, cellDetails,intersect,lis,flag){
      let row = intersect[0]; 
      let col = intersect[1];
  ...
  function shortestPath(lis){
    ...
    for(let i=1;i<lis.length;i++){
        let dis = Math.abs(lis[i][0]-lis[i-1][0])+Math.abs(lis[i][1]-lis[i-1][1]);
        shortestDist+=Math.sqrt(dis);
    }
