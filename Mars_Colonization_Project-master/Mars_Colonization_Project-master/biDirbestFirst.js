async function biDirbestFirstSearch(state, diagonalMovement = false){
    ...
    var hVal1 = heuristicVal([state.start.y, state.start.x], [state.end.y, state.end.x] , diagonalMovement, state.heuristic); 
    openList1.push([state.start.y , state.start.x, hVal1]);
    var hVal2 = heuristicVal([state.end.y, state.end.x], [state.start.y, state.start.x] , diagonalMovement, state.heuristic); 
    ...
    while((!openList1.isEmpty())&&(!openList2.isEmpty())){
        ...
        for(let i=0;i<neighbours1.length;i++){
            ...
            if((closedList1[ny1][nx1] === 0)&&(state.matrix[ny1][nx1] !== 1)){
                ...
                var newH1 = heuristicVal([ny1, nx1], [state.end.y, state.end.x], diagonalMovement, state.heuristic); 
        ...
        for(let i=0;i<neighbours2.length;i++){
            ...
            if((closedList2[ny2][nx2] === 0)&&(state.matrix[ny2][nx2] !== 1)){
                ...
                var newH2 = heuristicVal([ny2, nx2], [state.start.y, state.start.x], diagonalMovement, state.heuristic); 
    ...
    if(foundDest){
        let flag = 0;
        var lis = [];
        getPathtbest(state, intersect[1], intersect[0], parent1,lis,flag);
        flag = 1;
        getPathtbest(state, intersect[1], intersect[0],parent2,lis,flag);
  ...
  function getPathtbest(state, ex, ey, parent,lis,flag) {
    curr = [ey, ex];
